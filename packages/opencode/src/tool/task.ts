...
const BaseParameterFields = {
  ...
  provider: Schema.optional(Schema.String).annotate({
    description:
      'Local provider (host) to run this subagent on, e.g. "gpuhost1" or "gpuhost4". Use when the user names a host; copy the name exactly. Omit to auto-place on an idle host.',
  }),
}
...
export const TaskTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    // can honor "run this on gpuhost1". Discovery is async, so hosts appearing
