    ...
    Effect.gen(function* () {
      ...
      for (const svc of online) {
        ...
        if (svc.source === "lan") {
          ...
          yield* Effect.logInfo("added provider", { slug, baseURL: svc.baseURL, source: svc.source, defaultModel: svc.defaultModel })
          changed = true
          continue
        }
        ...
        if (slug in providers) {
          ...
          yield* Effect.logInfo("updated provider baseURL", { slug, old: oldURL, new: svc.baseURL, defaultModel: svc.defaultModel })
          changed = true
        } else {
          providers[slug] = {
            npm: "@ai-sdk/openai-compatible",
        ...
        if (urlOwner && urlOwner !== slug && isAutoDiscovered(providers[urlOwner])) {
          ...
          yield* Effect.logInfo("removed duplicate provider for same baseURL", { id: urlOwner, kept: slug, baseURL: svc.baseURL })
          changed = true
        }
      }
