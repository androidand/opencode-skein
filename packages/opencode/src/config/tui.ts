...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  if (flags.tuiConfig.value) {
    const configFile = flags.tuiConfig.value
  ...
  const configDir = flags.configDir.value
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opencode") || dir === configDir)
