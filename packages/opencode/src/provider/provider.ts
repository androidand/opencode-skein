...
export async function adjustLocalContextOnOverflow(
  ...
  try {
    ...
    if (peek?.error?.type === "context_too_large") {
      ...
      // VRAM-constrained host does not load (this is what set gpuhost2 to 393216 >
...
// worst case (254s to first token on a small prompt, gpuhost2 hybrid DeepSeek) with
...
async function fetchLocalModelFit(
  ...
  try {
    ...
    // accepts TCP but never answers /api/fit (gpuhost2 mid rootfs-swap) must not
