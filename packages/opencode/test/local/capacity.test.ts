...
describe("exact queue telemetry beats utilisation", () => {
  // The regression this module exists for. gpuhost2, measured 2026-07-26: 85% GPU
  // utilisation with an empty queue and the model resident. A scheduler reading
  // utilisation skips a completely free 48GB host.
  test("gpuhost2: high GPU utilisation with an empty queue is free", () => {
    const s = snap(
      hw({
        gpus: gpus(85),
  ...
  test("gpuhost1: a genuinely serving host is busy", () => {
  ...
  test("gpuhost3: low utilisation and empty queue is free", () => {
