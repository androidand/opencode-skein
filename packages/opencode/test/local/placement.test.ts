...
describe("bestModel context-adequacy filter", () => {
  ...
  test("excludes a tiny-ctx model even when it is resident, perfect-fit, and fastest (the gpuhost5 bias)", () => {
...
describe("bestModel host-paced placement (hybrid GPU + system RAM)", () => {
  ...
  // Measured on gpuhost2: the same host serves a full-GPU model at 70 tok/s and a
