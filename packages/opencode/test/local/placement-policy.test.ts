...
describe("shouldAttemptPlacement", () => {
  ...
  test("a role that says where it wants to run survives a cloud parent", () => {
    ...
    expect(shouldAttemptPlacement({ parentIsLocal: false, prefer: ["gpuhost1"] })).toBe(true)
  })

  test("an explicit host is honoured from anywhere", () => {
    expect(shouldAttemptPlacement({ parentIsLocal: false, target: "gpuhost1" })).toBe(true)
    expect(shouldAttemptPlacement({ parentIsLocal: false, target: "gpuhost1", prefer: "inherit" })).toBe(true)
  })
...
describe("hostRankFor", () => {
  test("earlier in the list outranks later, and both outrank unnamed", () => {
    const prefer = ["gpuhost1", "gpuhost4"]
    expect(hostRankFor(prefer, "gpuhost1")).toBeGreaterThan(hostRankFor(prefer, "gpuhost4"))
    expect(hostRankFor(prefer, "gpuhost4")).toBeGreaterThan(hostRankFor(prefer, "gpuhost2"))
    expect(hostRankFor(prefer, "gpuhost2")).toBe(0)
  })

  test("`local` and `inherit` express no host preference", () => {
    expect(hostRankFor("local", "gpuhost1")).toBe(0)
    expect(hostRankFor("inherit", "gpuhost1")).toBe(0)
    expect(hostRankFor(undefined, "gpuhost1")).toBe(0)
  })
  ...
  test("a preference never outweighs the host-paced penalty", () => {
    const prefer = ["gpuhost1", "gpuhost4", "gpuhost2", "gpuhost3", "gpuhost5"]
