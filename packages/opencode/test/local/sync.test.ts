...
const run = (
  ...
  reconcileProviders({
    ...
    selfSlug: opts?.selfSlug ?? "gpuhost5",
  })

describe("reconcileProviders", () => {
  test("adds a newly discovered mDNS provider", () => {
    const { providers, changes } = run({}, [mdns("gpuhost1", "192.0.2.126")])
    expect(providers.gpuhost1).toMatchObject({ options: { baseURL: "http://192.0.2.126:8080/v1", apiKey: "skein" } })
    expect(changes).toContainEqual({
      type: "added",
      slug: "gpuhost1",
      baseURL: "http://192.0.2.126:8080/v1",
      source: "mdns",
    })
  })

  test("updates an auto-discovered provider whose IP moved", () => {
    const { providers, changes } = run({ gpuhost1: auto("http://192.0.2.99:8080/v1") }, [mdns("gpuhost1", "192.0.2.126")])
    expect((providers.gpuhost1 as any).options.baseURL).toBe("http://192.0.2.126:8080/v1")
    expect(changes).toContainEqual({
      type: "updated",
      slug: "gpuhost1",
      from: "http://192.0.2.99:8080/v1",
      to: "http://192.0.2.126:8080/v1",
    })
  })

  test("preserves other fields of an auto-discovered entry when re-pointing it", () => {
    const existing = { ...auto("http://192.0.2.99:8080/v1"), name: "Gpuhost1", discoverModels: true }
    const { providers } = run({ gpuhost1: existing }, [mdns("gpuhost1", "192.0.2.126")])
    expect(providers.gpuhost1).toMatchObject({ name: "Gpuhost1", discoverModels: true, options: { apiKey: "skein" } })
  })
  ...
  test("never re-points a hand-written provider that happens to share a slug", () => {
    ...
    const { providers, changes } = run({ gpuhost4: manual("http://192.0.2.239:11435/v1") }, [mdns("gpuhost4", "192.0.2.5")])
    expect((providers.gpuhost4 as any).options.baseURL).toBe("http://192.0.2.239:11435/v1")
    expect(changes).toContainEqual({ type: "kept-manual", slug: "gpuhost4", baseURL: "http://192.0.2.239:11435/v1" })
  })

  test("leaving a hand-written provider alone is not a config mutation", () => {
    const { changes } = run({ gpuhost4: manual("http://192.0.2.239:11435/v1") }, [mdns("gpuhost4", "192.0.2.5")])
    expect(changes.every((change) => change.type === "kept-manual")).toBe(true)
  })

  test("an already-correct entry produces no changes at all", () => {
    const { changes } = run({ gpuhost1: auto("http://192.0.2.126:8080/v1") }, [mdns("gpuhost1", "192.0.2.126")])
    expect(changes).toEqual([])
  })

  test("removes an auto-discovered entry pointing at one of this host's own IPs", () => {
    const { providers, changes } = run({ gpuhost4: auto("http://192.0.2.218:8080/v1") }, [], {
      selfIPs: ["192.0.2.218"],
    })
    expect(providers.gpuhost4).toBeUndefined()
    expect(changes).toContainEqual({ type: "removed-own-ip", id: "gpuhost4", host: "192.0.2.218" })
  })

  test("never removes a hand-written entry pointing at an own IP", () => {
    const { providers, changes } = run({ gpuhost4: manual("http://192.0.2.218:8080/v1") }, [], {
      selfIPs: ["192.0.2.218"],
    })
    expect(providers.gpuhost4).toBeDefined()
    expect(changes).toEqual([])
  })

  test("keeps an own-IP entry that is this machine's own slug", () => {
    const { providers } = run({ gpuhost5: auto("http://192.0.2.218:8080/v1") }, [], {
      selfIPs: ["192.0.2.218"],
      selfSlug: "gpuhost5",
    })
  ...
  test("removes a stale auto-discovered duplicate squatting the same URL", () => {
    const { providers, changes } = run({ mac: auto("http://192.0.2.126:8080/v1") }, [mdns("gpuhost5", "192.0.2.126")])
  ...
  test("never removes a hand-written entry squatting the same URL", () => {
    const { providers } = run({ mac: manual("http://192.0.2.126:8080/v1") }, [mdns("gpuhost5", "192.0.2.126")])
    expect(providers.mac).toBeDefined()
    expect(providers.gpuhost5).toBeDefined()
  })

  test("a weak LAN name may add a brand-new entry", () => {
    const { providers } = run({}, [{ ...mdns("newbox", "192.0.2.30"), source: "lan" }])
    expect(providers.newbox).toBeDefined()
  })

  test("a weak LAN name never re-points an existing entry", () => {
    const { providers, changes } = run({ gpuhost1: auto("http://192.0.2.99:8080/v1") }, [
      { ...mdns("gpuhost1", "192.0.2.30"), source: "lan" },
    ])
    expect((providers.gpuhost1 as any).options.baseURL).toBe("http://192.0.2.99:8080/v1")
    expect(changes).toEqual([])
  })

  test("a weak LAN name never claims a URL another entry already owns", () => {
    const { providers, changes } = run({ gpuhost1: auto("http://192.0.2.30:8080/v1") }, [
      { ...mdns("othername", "192.0.2.30"), source: "lan" },
    ])
  ...
  test("services on one of this machine's own IPs are skipped entirely", () => {
    const { providers, changes } = run({}, [mdns("gpuhost5", "192.0.2.218")], { selfIPs: ["192.0.2.218"] })
