...
function svc(over: Partial<Record<string, unknown>> = {}) {
  return {
    ...
    host: "gpuhost5.local",
    port: 11435,
    baseURL: "http://gpuhost5.local:11435",
  ...
  test("collapses duplicate discoveries of one endpoint, preferring the one that answered", async () => {
    const hosts = await discoverGalleryHosts({
      scan: fakeScan([
        ...
        svc({ source: "mdns", online: true, models: ["qwen3-32b"], name: "gpuhost5" }),
      ]),
    })
    expect(hosts).toHaveLength(1)
    expect(hosts[0]!.name).toBe("gpuhost5")
    expect(hosts[0]!.online).toBe(true)
  })

  test("host ids are normalized so a trailing slash is not a second host", () => {
    expect(hostId("http://M5.local:11435/")).toBe(hostId("http://gpuhost5.local:11435"))
  })
...
function onlineHost(over: Partial<GalleryHost> = {}): GalleryHost {
  return {
    id: "http://gpuhost5:11435",
    name: "gpuhost5",
    baseURL: "http://gpuhost5:11435",
    source: "mdns",
    online: true,
    installedModelIDs: [],
    defaultModel: null,
...
describe("gallery join (5.3)", () => {
  ...
  test("an unreachable capacity probe leaves busy unknown rather than false", () => {
    ...
    const rows = joinGalleryRows({
      ...
      candidates: [{ candidateId: "c1" }],
      fits: [],
      capacity: [{ provider: "gpuhost5", baseURL: host.baseURL, reachable: false, probedAt: 0, ageMs: 0, stale: false }],
    })
  ...
  test("capacity is matched through the same normalization as host ids", () => {
    const rows = joinGalleryRows({
      ...
      capacity: [
        { provider: "gpuhost5", baseURL: host.baseURL + "/", reachable: true, busy: true, probedAt: 0, ageMs: 0, stale: false },
      ],
    })
