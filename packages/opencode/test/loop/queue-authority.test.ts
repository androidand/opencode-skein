...
describe("queue authority boundary — bypass attempts", () => {
  ...
  const attempts: Array<[label: string, command: string]> = [
    ...
    ["remote execution via ssh", "ssh gpuhost5 'cd repo && git push'"],
    ["remote execution via ssh -o", "ssh -o StrictHostKeyChecking=no gpuhost2 git push"],
    ["scp a script out", "scp payload.sh gpuhost5:/tmp/"],
    ["rsync out", "rsync -a . gpuhost5:/tmp/repo"],
...
describe("an unattended session is self-sufficient on permissions", () => {
  ...
  test("auto-approval never extends to the denied commands themselves", async () => {
    for (const command of ["git push origin main", "gh release create v1", "ssh gpuhost5 git push"]) {
      const result = await denied(command)
      expect(result.denied).toBe(true)
    }
  })
