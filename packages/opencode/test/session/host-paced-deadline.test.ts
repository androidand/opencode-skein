...
// on gpuhost2, 254s can pass before the FIRST token of any kind while ~50 GB of
...
describe("host-paced model registry", () => {
  ...
  test("a failed fit probe keeps the previous verdict; fresh data overrides", () => {
    Provider.noteHostPaced("gpuhost2", "big-moe", { hostPaced: true })
    expect(Provider.isHostPaced("gpuhost2", "big-moe")).toBe(true)

    // probe lost the race: no fit report for this pass
    Provider.noteHostPaced("gpuhost2", "big-moe", undefined)
    expect(Provider.isHostPaced("gpuhost2", "big-moe")).toBe(true)

    // re-placed fully GPU-resident: fresh data clears the flag
    Provider.noteHostPaced("gpuhost2", "big-moe", { hostPaced: false })
