...
test("never recommends above max_fit_ctx", () => {
  // The gpuhost2 case: plenty of free VRAM, but the model is trained to 32k so
  // llama-skein reports max_fit_ctx 32768. Uncapped this returned ~250k.
  expect(computeRecommendedCtx(mem({ freeMb: 1_000_000 }), 32768)).toBe(MAX_CTX)
  expect(computeRecommendedCtx(mem({ freeMb: 1_000_000 }), 32768, 32768)).toBe(32768)
})

test("the ceiling outranks the workflow floor", () => {
  // A 32k-trained model cannot reach MIN_WORKFLOW_CTX; recommending it anyway
...
test("PRESETS offers sizes above a typical trained ceiling", () => {
  // Guards the premise of the fix: the hand-authored preset list reaches far
  // past what a 32k model can load, so the ceiling annotation must exist.
