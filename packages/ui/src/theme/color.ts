...
export function oklchToRgb(oklch: OklchColor): { r: number; g: number; b: number } {
  ...
  const l3 = l * l * l
  const gpuhost4 = m * m * m
  const s3 = s * s * s

  const lr = 4.0767416621 * l3 - 3.3077115913 * gpuhost4 + 0.2309699292 * s3
  const lg = -1.2684380046 * l3 + 2.6097574011 * gpuhost4 - 0.3413193965 * s3
  const lb = -0.0041960863 * l3 - 0.7034186147 * gpuhost4 + 1.707614701 * s3
